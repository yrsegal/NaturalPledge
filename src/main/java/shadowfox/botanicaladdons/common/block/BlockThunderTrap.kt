    ...
    override fun onEntityCollidedWithBlock(worldIn: World, pos: BlockPos, state: IBlockState, entityIn: Entity) {
        if (entityIn is EntityLivingBase || entityIn is EntityItem) {
            if (!worldIn.isRemote) {
                ...
                val diffvec = Vector3.fromEntity(entityIn).sub(Vector3.fromBlockPos(pos)).multiply(5.0)
                ...
                for (i in 0..10) {
                    ...
                    Botania.proxy.lightningFX(worldIn, posVec, posVec.copy().add(fireVec), 0.5f, 0x00948B, 0x00E4D7)
                }
            }
