...
class PotionRooted(iconIndex: Int) : PotionMod(LibNames.ROOTED, true, 0x634D05, iconIndex, true) {
    ...
    override fun applyAttributesModifiersToEntity(entityLivingBaseIn: EntityLivingBase, p_111185_2_: AbstractAttributeMap?, amplifier: Int) {
        if (entityLivingBaseIn is EntityLiving)
            entityLivingBaseIn.setNoAI(true)
    }

    override fun removeAttributesModifiersFromEntity(entityLivingBaseIn: EntityLivingBase, p_111187_2_: AbstractAttributeMap?, amplifier: Int) {
        if (entityLivingBaseIn is EntityLiving)
