...
import shadowfox.botanicaladdons.common.integration.tinkers.traits.TraitWorldforged.Companion.isSemiDisposable
...
class TraitElsetouched : AbstractTrait("elsetouched", TinkersIntegration.ELEMENTIUM_COLOR) {
    ...
    companion object {
        ...
        fun summonPixie(summoner: EntityLivingBase, target: EntityLivingBase, damage: Float, useEffects: Boolean) {
            ...
            val pixie = EntityPixie(summoner.worldObj)
            pixie.setPosition(summoner.posX, summoner.posY + 2, summoner.posZ)

            if (useEffects)
                pixie.setApplyPotionEffect(PotionEffect(potions[summoner.worldObj.rand.nextInt(potions.size)], 40, 0))
